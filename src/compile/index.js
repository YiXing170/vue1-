...
// ELEMENT_NODE: 1元素节点  TEXT_NODE: 3 文本节点   其它节点不处理
function compileNode (node, vm) {
  const type = node.nodeType;
  // 便于扩展
  const compileMethods = new Map([[1, compileElement], [3, compileTextNode]]); // 生成编译方法map对象{1：compileElement，2：compileTextNode}
...
function compileElementAttrs (el, vm) {
  ...
  for (let attr of attrs) {
    const name = attr.name.trim(); 'v-text'
    const value = attr.value.trim(); 'count'
    if (RE.on.test(name)) {  // v-on
      onAttr(name, value);
    } else if (RE.bind.test(name)) { //v-bind
      bindAttr(name, value);
  ...
  function bindAttr (name, value) {
    ...
    // 过滤器
    applyFilters(value, descriptor);  // 应用过滤函数
    des.push(descriptor);
  }
